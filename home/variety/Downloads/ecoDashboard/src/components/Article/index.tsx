...
import { getArticle } from '@/redux/informationSlice';
...
const Articles: React.FC = () => {
  ...
  const handleRemoveFinally = async (id: string) => {
    const response = await API.removeArticle(id)
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="grid border-t border-stroke px-4 py-4.5 dark:border-strokedark grid-cols-8 md:px-6 2xl:px-7.5">
        ...
        <div className="col-span-4 md:col-span-3 flex items-center">
          <p className="font-medium">Judul Artikel</p>
        </div>
      ...
      {dataArticles?.length > 0 && dataArticles
      .filter((sub: any) => {
        if (search && search !== '') {
          return sub.name_article.toLowerCase().includes(search.toLowerCase());
        }
        return true;
      })
          ...
          </div>
          {
            Auth?.role === 'super-admin' ? (
              <div className="col-span-4 md:col-span-1 flex">
                <div className='flex'>
                  <div onClick={() => handleRemoveArticle(data?.article_id)} className='w-[34px] h-[34px] rounded-[6px] mr-2 bg-[red] cursor-pointer hover:brightness-[90%] active:scale-[0.98] p-1 text-white flex items-center justify-center'>
                    <FaTrash />
                  </div>
                  <div onClick={() => handleUpdateArticle(data)} className='w-[34px] h-[34px] bg-yellow-500 rounded-[6px] ml-2 cursor-pointer hover:brightness-[90%] active:scale-[0.98] p-1 text-white flex items-center justify-center'>
                    <FaPenAlt />
                  </div>
                </div>
              </div>
